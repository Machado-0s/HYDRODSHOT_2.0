...
hydrolib_ReturnCode hydrolib_RingQueue_Read4BytesLE(hydrolib_RingQueue *self, uint32_t *data, uint16_t shift)
{
    ...
    if (buffer_index == self->capacity - 1)
    {
       *data = (uint32_t)self->buffer[self->capacity - 1] |        // Byte 0 (P0)
            (uint32_t)self->buffer[0] << 8 |                    // Byte 1 (P1)
            (uint32_t)self->buffer[1] << 16 |                   // Byte 2 (P2)
            (uint32_t)self->buffer[2] << 24;                    // Byte 3 (P3)
    }
    else if (buffer_index == self->capacity - 2)
    {
        *data = (uint32_t)self->buffer[self->capacity - 2] |        // Byte 0 (P0)
            (uint32_t)self->buffer[self->capacity - 1] << 8 |   // Byte 1 (P1)
            (uint32_t)self->buffer[0] << 16 |                   // Byte 2 (P2)
            (uint32_t)self->buffer[1] << 24;                    // Byte 3 (P3)
    }
    else if (buffer_index == self->capacity - 3)
    {
        *data = (uint32_t)self->buffer[self->capacity - 3] |        // Byte 0 (P0)
            (uint32_t)self->buffer[self->capacity - 2] << 8 |   // Byte 1 (P1)
            (uint32_t)self->buffer[self->capacity - 1] << 16 |  // Byte 2 (P2)
            (uint32_t)self->buffer[0] << 24;                    // Byte 3 (P3)
    }
    else
    {
        *data = *(uint32_t *)(self->buffer + buffer_index);
