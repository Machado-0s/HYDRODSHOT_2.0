...
volatile uint16_t pid_target_speed_rpms[MOTORS_COUNT] = {0};
...
constinit hydrv::GPIO::GPIOLow ok_led(hydrv::GPIO::GPIOLow::GPIOC_port,
                                      13, hydrv::GPIO::GPIOLow::GPIO_Output);
constinit hydrv::GPIO::GPIOLow error_led(hydrv::GPIO::GPIOLow::GPIOC_port,
                                         14,
...
constinit hydrolib::logger::LogDistributor<hydrv::UART::UART<255, 255>>
    distributor(log_format, uart2);
constinit hydrolib::logger::Logger<
    hydrolib::logger::LogDistributor<hydrv::UART::UART<255, 255>>>
...
void getCommmands(void) {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    if (pid_target_speed_rpm_conversion[i] >= 100 &&
      ...
      pid_target_speed_rpms[i] = signed_val * 120;
    }
  }
  ...
  while (1) {
    ...
    if (telemetry_done_flag) {
      ...
      for (int m = 0; m < MOTORS_COUNT; m++) {
        ...
        uint16_t target_rpm = pid_target_speed_rpms[m];
        ...
        uint16_t new_command =
            pid_calculate_command(m, current_rpm, target_rpm, dt);
